...
export class AuthService {
  ...
  private generateToken(payload: { sub: number; role: UserRole; email?: string; username?: string }): string {
    return this.jwtService.sign(payload);
  }

  private buildAuthResponse(token: string, userId: number, role: UserRole, extra: Record<string, any> = {}) {
    return {
      access_token: token,
      token_type: 'bearer',
      user_id: userId,
  ...
  async loginStudent(email: string, password: string) {
    ...
    const token = this.generateToken({ sub: student.id, role: UserRole.STUDENT, email: student.email });
  ...
  async loginAdmin(username: string, password: string) {
    ...
    const token = this.generateToken({ sub: admin.id, role: UserRole.ADMIN, username: admin.username });
  ...
  async loginSchool(email: string, password: string) {
    ...
    const token = this.generateToken({ sub: school.id, role: UserRole.SCHOOL, email: school.email });
  ...
  async registerStudent(dto: RegisterStudentDto) {
    ...
    const schoolExists = await this.schoolRepo.findOne({ where: { id: dto.school_id } });
    if (!schoolExists) {
